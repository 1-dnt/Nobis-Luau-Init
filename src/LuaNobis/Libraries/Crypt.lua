return function(GLOBAL_ENV, MainServices)
	...
	Crypt.crypt.generatebytes = function(len)
		local Type = typeof(len)
		assert(Type == "number", "invalid argument #1 to 'generatebytes' (number expected, got " .. Type .. ")")
		assert(len >= 0, "invalid argument #1 to 'generatebytes' (negative size specified)")

		local bytes = buffer.create(len)

		for i = 1, len do
			local CharIndex = math.random(1, 64) - 1
			buffer.writeu8(bytes, i - 1, buffer.readu8(lookupValueToCharacter, CharIndex))
		end

		return buffer.tostring(raw_encode(bytes))
	end

	Crypt.crypt.generatekey = function()
		return Crypt.crypt.generatebytes(32)
	end

	Crypt.crypt.encrypt = function(data, key)
		local Type = typeof(data)
		assert(Type == "string", "invalid argument #1 to 'encrypt' (string expected, got " .. Type .. ")")
		local Type = typeof(key)
		assert(Type == "string", "invalid argument #2 to 'encrypt' (string expected, got " .. Type .. ")")

		local len = #data
		local klen = #key

		assert(len > 0, "invalid argument #1 to 'encrypt' (negative or cero size specified)")
		assert(klen > 0, "invalid argument #2 to 'encrypt' (negative or cero size specified)")

		local out = buffer.create(len)

		local dataBytes = buffer.fromstring(data)
		local keyBytes = buffer.fromstring(key)

		local state = 0
		for i = 1, klen do
			state = bit32.bxor(state, buffer.readu8(keyBytes, i - 1) + len)
			state = bit32.lrotate(state, i % 8)
		end
		...
		for i = 0, (len - 1) do
			local mix = bit32.bxor(state, buffer.readu8(keyBytes, keyIndex - 1))
			mix = bit32.lrotate(mix, rot)

			local outByte = bit32.bxor(buffer.readu8(dataBytes, i), mix)
			buffer.writeu8(out, i, outByte)

			state = bit32.bxor(state, mix)
			state = bit32.lrotate(state, 3)

			keyIndex += 1
			rot += 1

			if keyIndex > klen then
				keyIndex = 1
			end
		...
		return buffer.tostring(out), key
	end
	
	Crypt.crypt.decrypt = function(data, key)
		local Type = typeof(data)
		assert(Type == "string", "invalid argument #1 to 'decrypt' (string expected, got " .. Type .. ")")
		local Type = typeof(key)
		assert(Type == "string", "invalid argument #2 to 'decrypt' (string expected, got " .. Type .. ")")

		local len = #data
		local klen = #key

		assert(len > 0, "invalid argument #1 to 'decrypt' (negative or cero size specified)")
		assert(klen > 0, "invalid argument #2 to 'decrypt' (negative or cero size specified)")
